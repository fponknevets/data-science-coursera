...
Prefixes for functions for these four types of ditribtion are:
...
```rnorm``` generates random **Normal** variates given a mean and standard deviation

```R
# rnorm(x, m, sd)  where x = number of variate to generate, m = mean, sd = std dev
x <- rnorm(100, 50, 10)
```
